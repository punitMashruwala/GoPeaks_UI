...
export class TextSelectDirective implements OnInit, OnDestroy {
  ...
  private isRangeFullyContained(range: Range): boolean {
    ...
    while (selectionContainer.nodeType !== Node.ELEMENT_NODE) {
      ...
      if(selectionContainer.parentNode != null) selectionContainer = selectionContainer.parentNode;
  ...
  private processSelection(): void {
    ...
    let range;
    let rangeContainer;
    let localRectangle: any;
    let viewportRectangle: any;
    if (selection != null) {
      range = selection.getRangeAt(0);
    }
    if (range != null) rangeContainer = this.getRangeContainer(range);
    ...
    if (this.elementRef.nativeElement.contains(rangeContainer)) {
      ...
      if (range != null) viewportRectangle = range.getBoundingClientRect();
      if (viewportRectangle != null && rangeContainer != null) localRectangle = this.viewportToHost(viewportRectangle, rangeContainer);
  ...
  private viewportToHost(
    ...
    // local position.
      ...
      } while ((node !== host) && (node.parentNode !== null) && (node = node.parentNode));
